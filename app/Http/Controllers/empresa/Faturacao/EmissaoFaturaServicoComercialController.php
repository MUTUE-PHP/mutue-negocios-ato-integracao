...
//    }
    ...
    public function updatedFaturaQtdMeses($qtdMeses){
        $dataEntrada = $this->fatura['dataEntradaEstacionamento'];
        if($dataEntrada && $qtdMeses){
            $dataFinal = Carbon::parse($dataEntrada)->addMonths($qtdMeses);
            $this->fatura['dataSaidaEstacionamento'] =(string)$dataFinal->format('Y-m-d\TH:i');
        }
    }
    public function updatedFaturaDataEntradaEstacionamento($dataEntrada){
        $qtdMeses = $this->fatura['qtdMeses'];
        if($dataEntrada && $qtdMeses){
            $dataFinal = Carbon::parse($dataEntrada)->addMonths($qtdMeses);
            $this->fatura['dataSaidaEstacionamento'] =(string)$dataFinal->format('Y-m-d\TH:i');
        }
