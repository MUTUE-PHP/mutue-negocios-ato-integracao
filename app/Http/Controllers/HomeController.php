       ...
       // dd('teste');
        ...
        $data['produtoMaisVendido'] = DB::select('
          ...
          SUM(factura_items.total) AS total_preco
                 ...
                 WHERE facturas.empresa_id = "' . $empresa['empresa']['id'] . '" AND facturas.tipo_documento = 1 AND facturas.anulado = "N"
...
//                $query->where('tipo_documento', $TIPO_FACTURA)
        ...
        $data['countproforma'] = DB::connection('mysql2')->table('facturas')
            ->where('empresa_id', $empresa['empresa']['id'])
            ->where('anulado', "N")
            ->where(function ($query) use ($TIPO_FACTURA_PROFORMA) {
//                $query->where('tipo_documento', $TIPO_FACTURA)
        ...
        $data['counttotalvendas'] = DB::connection('mysql2')->table('facturas')
            ->where('empresa_id', $empresa['empresa']['id'])
            ->where('anulado', "N")
            ->where(function ($query) use ($TIPO_FACTURA, $TIPO_FACTURA_RECIBO) {
//                $query->where('tipo_documento', $TIPO_FACTURA)
                $query->where('tipo_documento', $TIPO_FACTURA_RECIBO);
            })->sum('total');
        ...
        $data['counttotalproforma'] = DB::connection('mysql2')->table('facturas')
            ->where('empresa_id', $empresa['empresa']['id'])
            ->where('anulado', "N")
            ->where(function ($query) use ($TIPO_FACTURA, $TIPO_FACTURA_PROFORMA) {
//                $query->where('tipo_documento', $TIPO_FACTURA)
                $query->where('tipo_documento', $TIPO_FACTURA_PROFORMA);
            })->sum('total');
        ...
        $data['counttotalfactura'] = DB::connection('mysql2')->table('facturas')
            ->where('empresa_id', $empresa['empresa']['id'])
            ->where('anulado', "N")
            ->where(function ($query) use ($TIPO_FACTURA) {
//                $query->where('tipo_documento', $TIPO_FACTURA)
                $query->where('tipo_documento', $TIPO_FACTURA);
            })->sum('total');


        $data['tiposservicos'] = DB::connection('mysql2')->table('tipos_servicos')->get();

//        dd($data['tiposservicos']);
